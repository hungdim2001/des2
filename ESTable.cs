...
namespace Ma_Hoa
{
    ...
    {28, 29, 30, 31, 32, 1}};
        ...
        public string DoETable(int[] Right_in, int[] Right_out)
        {
            string temp_result = " ";
            int temp = 0;
            int i = 0;
            ...
            temp_result += "\n";
             temp_result += " E- table ";
            for (int j = 0; j < 48; j++)
                temp_result+=Right_out[j];
            int index = 0;
            for (int g = 0; g < 6; g++)
            {
                for (int j = 0; j < 8; j++)
                {
                    RightS[g,j] = Convert.ToString(Right_out[index]);
                    index++;
                }
            }
            temp_result+= Arrayprinter.output_Array(RightS, "After E-Table");
            index = 0;
            return temp_result;
        }
